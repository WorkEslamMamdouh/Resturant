...
//        }
        ...
        [HttpGet]
        public IHttpActionResult GetNotifications(int comCode, int BraCode, string SystemCode, string SubSystemCode)
        {
            //if (ModelState.IsValid)
            //{
            //    var not = db.G_Noteifications.Where(x => x.SYSTEM_CODE == SystemCode && x.SUB_SYSTEM_CODE == SubSystemCode);
            //    var notCom = db.G_NotificationCompany.Where(x => x.SYSTEM_CODE == SystemCode && x.SUB_SYSTEM_CODE == SubSystemCode && x.CompCode == comCode && x.BranchCode == BraCode);
