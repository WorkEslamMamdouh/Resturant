...
namespace Home {
    ...
    function btnChange_onclick() {
        debugger;
        if (TR_Type == "2") { TR_Type = "1"; btnChange.innerText = "الذهب الي حجز الاطفال"; }
        else { TR_Type = "2"; btnChange.innerText = " الذهب الي حجز الرجال"; }
    ...
    function Assign_Insert() {
        ...
        if (txt_Cust_Type.value == "Null") {
            ...
            if (txt_NAME.value == "") { Name = "Samah" } else { Name = txt_NAME.value; }
            if (txt_MOBILE.value == "") { Phone = "Samah" } else { Phone = txt_MOBILE.value; }
        ...
        if (veid != 1) {
            Ajax.Callsync({
                ...
                success: (d) => {
                    ...
                    if (result.IsSuccess) {
                        debugger;
                        BilldIData = result.Response as Array<Table_Hagz>;
    ...
    function Details_Insert(BilldIData: Array<Table_Hagz>) {
        debugger
        ID += 1;
    ...
    function Delete_Rows(cnt: number) {
    ...
    function DisplayData(Selecteditem: Array<Table_Hagz>) {
    ...
    function Enter_Rows(cnt: number) {
